...
function quickModeRoll() {
  ...
  if (!isAutoFail && !isGauntlet) {
    ...
    const fillPct = manualSaturation; // already 0-100%
    ...
    if (roll < successChance) {
      ...
      if (closeCall) {
        ...
        manualSaturation = Math.min(manualSaturation + cc.sat, 100);
...
function reportEmergencyLeak() {
  if (profileMode === 'babysitter') {
    ...
    openStatusModal(function() {
      ...
      const leakThrough = sat >= 100;
      const fillPct = sat; // sat is already 0-100%
...
function checkOverflowSaturation(val) {
  let s = parseInt(val);
  if (s >= 100 && !window._overflowFired) {
    ...
    // Reset flag when saturation drops below 100% (e.g. after a change)
